...
{
    public class AccountNoteListPresenter : Presenter
    {
        IAccountNoteListView _view;

        public AccountNoteListPresenter(IAccountNoteListView view, ISessionProvider session, ISecurityContext securityContext)
            : base(view, session, securityContext)
        {
            _view = base.GetView<IAccountNoteListView>();
        ...
        void GetNoteResults(InsightGridArg e)
        {
            int count = 0;
            _view.ResultSet = new AccountNoteServices().
