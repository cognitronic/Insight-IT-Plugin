...
{
    public class AccountAddressListPresenter : Presenter
    {
        IAccountAddressListView _view;

        public AccountAddressListPresenter(IAccountAddressListView view, ISessionProvider session, ISecurityContext securityContext)
            : base(view, session, securityContext)
        {
            _view = base.GetView<IAccountAddressListView>();
        ...
        {
            int count = 0;
            _view.ResultSet = new AccountAddressServices().
                GetByAccountID(GetCurrentItemReference<Account>().ID).
                ToList<AccountAddress>();
