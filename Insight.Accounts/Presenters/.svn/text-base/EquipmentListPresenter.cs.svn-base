...
namespace Insight.Accounts.Presenters
{
    public class EquipmentListPresenter : Presenter
    {
        ...
        {
            string url = SecurityContextManager.Current.BaseURL + "/Accounts/ID=" + e.ObjectName.Replace(" ", "-") + "/Equipment/ID=" + e.ObjectID.ToString();
            _view.NavigateTo(url);
        }
        ...
        void GetResults(InsightGridArg e)
        {
            if (!IsInsert<Account>())
            {
                _view.ResultSet = new EquipmentServices().
