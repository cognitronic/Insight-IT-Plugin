...
{
    public class AccountAddressPropertiesPresenter : Presenter
    {
        IAccountAddressPropertiesView _view;

        public AccountAddressPropertiesPresenter(IAccountAddressPropertiesView view, ISessionProvider session, ISecurityContext securityContext)
            : base(view, session, securityContext)
        {
            _view = base.GetView<IAccountAddressPropertiesView>();
        ...
        void SaveAddress()
        {
            ...
            new AccountAddressServices().Save(address);
            if (isInsert)
                _view.NavigateTo(url + address.ID.ToString());
            else
                _view.LoadAddress(address);
        }
