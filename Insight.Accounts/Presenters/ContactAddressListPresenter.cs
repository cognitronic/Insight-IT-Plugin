...
{
    public class ContactAddressListPresenter : Presenter
    {
        IContactAddressListView _view;

        public ContactAddressListPresenter(IContactAddressListView view, ISessionProvider session, ISecurityContext securityContext)
            : base(view, session, securityContext)
        {
            _view = base.GetView<IContactAddressListView>();
        ...
        {
            var item = (ContactAddress)e.Item;
        ...
        {
            int count = 0;
            _view.ResultSet = GetCurrentItemReference<Contact>().ContactAddress;
